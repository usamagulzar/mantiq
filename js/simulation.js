            ...
            if (isConst) {
                ...
                svgContent += `
                    <g class="sim-toggle" data-var="${varName}" style="cursor:pointer;">
                        ...
                        ${statusDot}
                        <!-- Label -->
                        <text x="${x-35}" y="${y}" font-family="Outfit,sans-serif" font-size="18" font-weight="900" fill="#fff" text-anchor="end" dominant-baseline="central" stroke="#1A4E2C" stroke-width="3" paint-order="stroke fill">${typeof formatSubscriptUnicode === 'function' ? formatSubscriptUnicode(varName) : escapeHtml(varName)}</text>
                    </g>
                `;
            }
        } else {

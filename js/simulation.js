function generateSVGForSimulation(root, panelId = 'p', panelType = 'simOrig') {
    ...
    // Extract unique master variable names from depthGroups[0]
    const uniqueVars = [];
    if (depthGroups[0]) {
        for (const n of depthGroups[0]) {
            if (!n.isGate && n.value !== '0' && n.value !== '1') {
                if (!uniqueVars.includes(n.value)) {
                    uniqueVars.push(n.value);
                }
            }
        }
    }

    const masterBtnX = -180;
    const busSpacing = 16;
    const numUnique = uniqueVars.length;
    
    let minLeafY = 0;
    let maxLeafY = 0;
    if (depthGroups[0] && depthGroups[0].length > 0) {
        minLeafY = posMap.get(depthGroups[0][0]).y;
        maxLeafY = posMap.get(depthGroups[0][depthGroups[0].length - 1]).y;
    }

    const masterBtnY = {};
    uniqueVars.forEach((vName, idx) => {
        if (numUnique === 1) {
            masterBtnY[vName] = (minLeafY + maxLeafY) / 2;
        } else {
            const step = (maxLeafY - minLeafY) / Math.max(1, numUnique - 1);
            masterBtnY[vName] = minLeafY + idx * step;
        }
    });

    let contentMinX = masterBtnX - 50;
    ...
    Object.values(masterBtnY).forEach(y => {
        contentMinY = Math.min(contentMinY, y - 35);
        contentMaxY = Math.max(contentMaxY, y + 35);
    });
    ...
    // --- DRAW MASTER VARIABLE BUTTONS (Wirelessly linked to circuit nodes) ---
    uniqueVars.forEach((vName, idx) => {
        const xBtn = masterBtnX + dx;
        const yBtn = masterBtnY[vName] + dy;
        const state = !!simInputStates[vName];
        const capCY = state ? yBtn : yBtn - 4;

        const statusDot = `<circle id="toggle-master-dot-${panelId}-${vName}" cx="${xBtn+13}" cy="${yBtn-13}" r="3.5" fill="${state ? '#30d158' : '#ff453a'}" filter="url(#led-glow-small-${panelId})"/>`;
        svgContent += `
            <g class="sim-toggle" data-var="${vName}" style="cursor:pointer;">
                <rect x="${xBtn-24}" y="${yBtn-5}" width="5" height="10" rx="1.5" fill="url(#metal-pin-${panelId})" opacity="0.85"/>
                <rect x="${xBtn+19}" y="${yBtn-5}" width="5" height="10" rx="1.5" fill="url(#metal-pin-${panelId})" opacity="0.85"/>
                <rect x="${xBtn-22}" y="${yBtn-22}" width="44" height="44" rx="6" fill="#151515" filter="url(#plastic-3d-${panelId})"/>
                <circle id="toggle-master-cap-${panelId}-${vName}" cx="${xBtn}" cy="${capCY}" r="13" fill="#333" filter="url(#btn-cap-3d-${panelId})"/>
                ${statusDot}
                <text x="${xBtn-32}" y="${yBtn}" font-family="Outfit,sans-serif" font-size="18" font-weight="900" fill="#fff" text-anchor="end" dominant-baseline="central" stroke="#1A4E2C" stroke-width="3" paint-order="stroke fill">${typeof formatSubscriptUnicode === 'function' ? formatSubscriptUnicode(vName) : escapeHtml(vName)}</text>
            </g>
        `;
    });

    // --- DRAW COPPER TRACES FOR GATES ---
    {
        let traceIdx = 0;
        for (const [node, pos] of posMap.entries()) {
    ...
    for (const [node, pos] of posMap.entries()) {
        ...
        if (!node.isGate) {
            ...
            if (isConst) {
                ...
                svgContent += `<circle cx="${x}" cy="${y}" r="9" fill="none" stroke="#ffffff" stroke-width="1.2" opacity="0.6" filter="url(#silkscreen-${panelId})"/>`;
            }
        } else {
    ...
    // --- DRAW COMPONENTS (ICs & Small Leaf Connection Nodes) ---
    let componentIdx = 0;
    for (const [node, pos] of posMap.entries()) {
        ...
        if (!node.isGate) {
            ...
            if (isConst) {
                ...
                const nodePadId = `node-pad-${panelId}-${myIdx}`;
                const formattedLabel = typeof formatSubscriptUnicode === 'function' ? formatSubscriptUnicode(varName) : escapeHtml(varName);
                svgContent += `
                    <!-- Small Leaf Input Connection Node Pad -->
                    <circle cx="${x}" cy="${y}" r="7" fill="url(#metal-pad-${panelId})" filter="url(#plastic-3d-${panelId})"/>
                    <circle id="${nodePadId}" cx="${x}" cy="${y}" r="3.5" fill="${state ? '#60ff60' : '#225522'}"/>
                    <text x="${x - 12}" y="${y}" font-family="Outfit,sans-serif" font-size="13" font-weight="700" fill="#a5d6a7" text-anchor="end" dominant-baseline="central">${formattedLabel}</text>
                `;
            }
        } else {
            // Logic Gate IC
